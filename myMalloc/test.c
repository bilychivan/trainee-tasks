...
#define MEMORY_HEADER "myMalloc.h"
#define MEMORY_START initGlobalMemory
#define MEMORY_STOP freeGlobalMem
#define MEMORY_ALLOC requestMemory
#define MEMORY_REALLOC reallocMemory
#define MEMORY_FREE freeMemory
...
#define GLOBAL_MEMORY_ALLOCATION 4096
...
void memory_test(int number, int iteration) {
    ...
    fill(chunk, size, (char) number);

    print(chunk, size, 0, number, iteration);
    ...
    print(chunk, size, 1, number, iteration);
...
int main() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
       pthread_create(&thread[i], NULL, test, (void*)i);
    }
