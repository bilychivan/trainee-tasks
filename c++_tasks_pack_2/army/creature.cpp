...
Creature::Creature(std::string argName) : alive(true), corpse(true)
{
    ...
    {
        stats.push_back({0, Creature::StatUndefined});
    }
}
...
void Creature::Attack(Creature& target, float damageMultiplier)
{
    if (alive == false)
    {
...
void Creature::ReceiveDamage(DamageType type, float amount, Creature& attacker, bool canCounterattack)
{
    if (isAlive() == false)
    {
    ...
    if (attacker.CanBeCounterattacked() && canCounterattack && isAlive())
    {
...
void Creature::Counterattack(Creature& target)
{
