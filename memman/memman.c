...
void* mm_realloc(void* ptr, mm_size new_size)
{
    ...
    mm_size required_size = new_size + sizeof(mm_node_meta);

    if (node_meta->owner_node->unoccupied == FALSE)
    {
        if (node_meta->owner_node->length == required_size)
        {
            pthread_mutex_unlock(&mm_node_lock);
        ...
        if (node_meta->owner_node->length > required_size)
        {
            ...
            new_node->unoccupied = TRUE;
            new_node->length = node_meta->owner_node->length - required_size;
            new_node->next = (node_meta->owner_node->next != 0) ? node_meta->owner_node->next : 0;
#ifdef MM_DEBUG
            new_node->debug_offset = node_meta->owner_node->debug_offset + required_size;
#endif

            node_meta->owner_node->next = new_node;
            node_meta->owner_node->length = required_size;
        ...
        else
        {
            ...
            void* new_ptr = mm_alloc(required_size);
            if (new_ptr == 0)
            {
                pthread_mutex_unlock(&mm_node_lock);
