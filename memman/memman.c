...
static void mm_defragment(mm_node* target)
{
    ...
    if (target->unoccupied == FALSE)
    {
        mm_error("defragmentation of occupied blocks is forbidden\n");
        exit(1);
    }

    if (target->next != 0)
    {
        while ((target->next != 0) && ((target->next->unoccupied == TRUE)))
        {
            mm_node* del = target->next;

            target->length += target->next->length;

            if (target->next->next != 0)
            {
    ...
    if (target->prev != 0)
    {
        if (target->prev->unoccupied == TRUE)
        {
            target->prev->length += target->length;
...
void mm_free(void* ptr)
{
    ...
    if (node_meta->owner_node->unoccupied == FALSE)
    {
        ...
        mm_defragment(node_meta->owner_node);
    }
    else
    {
...
void* mm_realloc(void* ptr, mm_size new_size)
{
    ...
    if (node_meta->owner_node->unoccupied == FALSE)
    {
        ...
        if (node_meta->owner_node->length > new_size)
        {
            ...
            new_node->prev = node_meta->owner_node;
            #ifdef MM_DEBUG
        ...
        else
        {
            ...
            memcpy(new_ptr, ptr, node_meta->owner_node->length - sizeof(mm_node_meta));
...
void mm_debug_nodes()
{
    ...
    {
        #ifdef MM_DEBUG
            printf("Node (%lX) (%ld; %s; %ld; %lX; %lX)\n", (mm_size)tmp, tmp->debug_offset, (tmp->unoccupied == TRUE) ? "FREE" : "NOTF", tmp->length, (mm_size)tmp->prev, (mm_size)tmp->next);
        #else
            printf("Node (%lX) (%s; %ld; %lX; %lX)\n", (mm_size)tmp, (tmp->unoccupied == TRUE) ? "FREE" : "NOTF", tmp->length, (mm_size)tmp->prev, (mm_size)tmp->next);
        #endif
    }
